...
class Mortgage extends Asset
{
  ...
  private float	currentMonthlyIncome;	// Monthly income of mortgagees
  private String monthlyIncomeUpdated;	// date Monthly income updated
  ...
  static final float MONTHS_IN_YEAR = (float) 12.0;
  ...
  public float getCurrentWMonthlyIncome () { return currentMonthlyIncome; }
  public void  setCurrentMonthlyIncome (float c) { currentMonthlyIncome = c; }
  @Deprecated
  ...
  public String  getMonthlyIncomeUpdated () { return monthlyIncomeUpdated; }
  public void  setMonthlyIncomeUpdated (String w) { monthlyIncomeUpdated = w; }
  ...
  public float totalMonthlyNetPayments ()
  //
  // Computes the net total monthly payments made by the mortgagees, that is,
  // the expected total monthly mortgage amount less the expected total monthly grants.
  //
  {
    try
    {
      ...
      File mortgageFile = new File ("mortgage.dat");    // file of mortgage records
      float expectedTotalMonthlyMortgages = (float) 0.0; // expected total Monthly mortgages
      float expectedTotalMonthlyGrants = (float) 0.0;    // expected total Monthly grants
      float capitalRepayment = (float) 0.0;		// capital repayment
      float interestPayment = (float) 0.0;	        // interest payment
      float escrowPayment = (float) 0.0;	        // escrow payment
      float tempMortgage = (float) 0.0;	                // temporary mortgage payment
	...
	while (inFile.getFilePointer () != inFile.length ())
	{
          ...
          capitalRepayment = price / NUMBER_OF_MORTGAGE_PAYMENTS;
          interestPayment = mortgageBalance * INTEREST_RATE / MONTHS_IN_YEAR ;
          escrowPayment = (annualPropertyTax + annualInsurancePremium) / MONTHS_IN_YEAR;
          tempMortgage = capitalRepayment + interestPayment + escrowPayment;
          expectedTotalMonthlyMortgages += tempMortgage;
          maximumPermittedMortgagePayment = currentMonthlyIncome * MAXIMUM_PERC_OF_INCOME;

	  if (tempMortgage > maximumPermittedMortgagePayment)
		expectedTotalMonthlyGrants += tempMortgage - maximumPermittedMortgagePayment;
	}
	...
	inFile.close ();
      ...
      return (expectedTotalMonthlyMortgages - expectedTotalMonthlyGrants);

    }
    catch (Exception e)
    {
	System.out.println ("***** Error: Mortgage.totalMonthlyNetPayments () *****");
	System.out.println ("\t" + e);
	return ( (float) 0.0);
    }

  }  // totalMonthlyNetPayments
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  //
  {
	...
	currentMonthlyIncome = tempFloat.floatValue ();
	i++;
	...
	monthlyIncomeUpdated = input.toString ();
	i++;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  //
  {
	...
	fileName.writeBytes (currentMonthlyIncome + "|" + monthlyIncomeUpdated.toString () + "|");
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  //
  {
    ...
    System.out.println ("Current monthly income: " + currentMonthlyIncome);
    System.out.println ("	Date monthly income updated: " + monthlyIncomeUpdated);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  //
  {
	...
	System.out.println ("Enter current monthly income: ");
	input = UserInterface.getString();
	tempFloat = new Float (input.toString ());
	currentMonthlyIncome = tempFloat.floatValue ();

        System.out.println ("Enter today's date (mm/dd/yy):");
        monthlyIncomeUpdated = UserInterface.getString();
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  public void updateMonthlyIncome ()
  {
    ...
    System.out.println ("\n\nOld monthly income: " + currentMonthlyIncome);
    System.out.println ("Enter current monthly income:");
