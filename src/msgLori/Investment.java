...
class Investment extends Asset
{
  ...
  public float totalMonthlyReturnOnInvestment ()
  //
  // totalMonthlyReturnOnInvestment reads records from the investments
  // file and computes totalAnnualReturn by summing each expectedAnnualReturn
  // on investments and dividing by MONTHS_IN_YEAR.
  //
  {
    try
    {
	...
	return (totalAnnualReturn / (float) 12.0);
    }
    catch (Exception e)
    {
	System.out.println ("***** Error: Investment.totalMonthlyReturnOnInvestment () *****");
	System.out.println ("***** Returned a value of 0 *****");
	System.out.println ("\t" + e);
	return (float) 0.0;
    }

  }  // totalMonthlyReturnOnInvestment
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  {
	  try {
		  String linha = "Investment ID: " + padRight(assetNumber, 15) +
          "Investment Name: " + padRight(investmentName, 15) +
          "Expected return: " + padRight(Float.toString(expectedAnnualReturn), 12) +
          "Date expected return was updated: " + padRight(expectedAnnualReturnUpdated, 10);
